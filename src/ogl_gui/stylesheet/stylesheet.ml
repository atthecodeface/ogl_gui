...
(*t t_stylable_type *)
type t_stylable_type = St_float_6 | St_float_3 | St_float | St_int | St_int_3 | St_int_6 | St_rgb
...
(*a Style_id_hash, Style_id, Style_ids, Style, Stylable_desc - immutable after construction, but could be fully immutable *)
...
module Style = struct
  ...
  let str t =
    let str_svo acc svo =
      ...
      sfmt "%s%s:%s:%b\n" acc (Style_id.str sid) (str_of_svalue svalue) opt
    in
...
(*m Stylable_desc - immutable *)
module Stylable_desc = struct
  (*t Structure *)
  type t= {
      state_descriptor : ( string * ((string * int) list)) list; (* each entry is a state_class -> (list of state_name of state_class->int) mappings *)
      styles : (string * t_stylable_type) list; (* List of all stylenames the stylable cares about; usually a static list *)
    }
...
(*m Stylable_desc_built -  immutable *)
module Stylable_desc_built = struct
  (*t Structure *)
  type t= {
      desc : Stylable_desc.t;
      sids : Style_id.t array; (* Array of all style_ids the stylable cares about; created after the stylesheet is set up (effectively binds the desc to the stylesheet) *)
    }
  ...
  let create desc ids =
    ...
    let t = {
        desc;
        sids=Array.of_list (List.fold_left fn [] desc.Stylable_desc.styles);
      }
    in
...
(*a Stylable_value_ref - utterly mutable *)
(*m Stylable_value_ref *)
module Stylable_value_ref = struct 
  type t_stylable_value_ref = 
    Ref of t_stylable_value
  | Default
  | Inherit
  type t = { 
...
let svr_zero = Stylable_value_ref.create ()
    
(*a Stylable and Stylesheet - immutable post-construction *)
(*m Stylable - mutable parent, style_change_callback; else immutable *)
module rec Stylable : sig
  (*m Signature *)
  (*t structure and t_style_change_callback *)
  type t_style_selector = (t -> bool)
  and t_style_change_callback = (Style_id.t * t_stylable_value) list -> unit
  ...
  and t= {
      ...
      type_name : string;
      classes : string list;
      values    : Stylable_value_ref.t array; (* next_values.(i) corresponds to desc.sids.(i) *)
    }

  val create : Stylable_desc.t -> Stylesheet.t -> string -> (string * string) list -> t_style_change_callback -> t list -> t
  ...
  val apply_styles : int -> (Style_id.t * t_stylable_value) list -> t -> unit
  ...
  let get_nth_sid n t = 
    if n<t.num_base_styles then
     Stylable_desc_built.get_nth_sid n t.desc_built
   else
     t.extra_sids.(n-t.num_base_styles)

  (*f find_sid_index *)
  let find_sid_index sid t =
    match (Stylable_desc_built.find_sid_index sid t.desc_built) with
      Some sid -> Some sid
    | None -> (
      let find_sid acc i =
        let (n, opt_res) = acc in
  ...
  let find_sid_index_exn sid t =
    match find_sid_index sid t with 
      None -> raise (Stylable_desc_built.Style_id_not_found_in_binding (Style_id.str sid))
    | Some index -> index
  ...
  let get_value t (s:string) =
    Stylable_value_ref.get_value (get_value_ref t s)
  ...
  let create desc sheet type_name name_values style_change_callback children =
    ...
    let count_extra_styles acc nv =
      ...
      match Stylesheet.style_id_of_name name sheet with
        None -> acc
      | Some sid -> (
        match Stylable_desc_built.find_sid_index sid desc_built with
          None -> (acc+1)
        | Some sid_index -> acc
      )
    in
    let num_extra_styles = List.fold_left count_extra_styles 0 name_values in
    let num_base_styles = (Array.length desc_built.Stylable_desc_built.sids) in
    ...
    let t = {
        ...
        state      = Array.make (List.length desc.Stylable_desc.state_descriptor) 0;
        values     = Array.init num_styles (fun i -> Stylable_value_ref.create ());
      }
    in
    Stylesheet.add_stylable t sheet;
    ...
    let set_default_value nv =
      ...
      | Some sid -> (
        ...
        | Some sid_index -> (
          ...
          Stylable_value_ref.set_default_from_string t.values.(sid_index) stype value
        )
      )
    in
    ...
    let set_inheritance n vr =
      ...
      Stylable_value_ref.set_default_inherit vr di
    in
  ...
  let rec reset_next_values t =
    Array.iter Stylable_value_ref.reset t.values;
  ...
  let apply_styles l styles t = 
    let apply_style sid_sv =
      ...
      match find_sid_index sid t with
        None -> ()
      | Some sindex ->
         Stylable_value_ref.apply l sv t.values.(sindex) 
    in
    ...
    let value_ref =
      match find_sid_index sid t with
        ...
        if (Stylesheet.is_default_inherit sid t.sheet) then
          Stylable_value_ref.Inherit
        else
          Stylable_value_ref.Default
      )
      | Some sindex ->
         (
           Stylable_value_ref.next_value_ref t.values.(sindex)
         )
     in
     match value_ref with
       Stylable_value_ref.Ref v -> v
     | Stylable_value_ref.Default -> Stylesheet.get_default_value sid t.sheet
     | Stylable_value_ref.Inherit ->
        (
          match t.parent with
  ...
  let rec update_current_values_from_next t =
    let update_nth_value n acc =
      ...
      if (value == Stylable_value_ref.get_value t.values.(n)) then acc
      else (
        (*Printf.printf "update_nth_value %s : %s : %s \n%!" t.id_name (Style_id.str sid) (str_of_svalue value);*)
        Stylable_value_ref.set_value t.values.(n) value;
        (sid,value)::acc
      )
    in
...
and Style_rule : sig 
  ...
  val create : Stylable.t_style_selector list -> (Style_id.t * t_stylable_value) list -> t
  ...
  let apply t stylesheet =
    let apply_style e =
      Stylable.apply_styles (List.length t.selectors) t.styles e
    in
    let rec sel_cbk_for_remaining_rules rules =
      match rules with
      | hd::nxt::tail -> 
         let (sel,cbk) = sel_cbk_for_remaining_rules (nxt::tail) in
         (hd,  fun e -> Stylable.element_callback_matching_subelements sel e stylesheet cbk)
...
and Stylesheet : sig 
  ...
  val add_style_defaults : t -> (string * t_stylable_value * bool) list -> unit
  val element_callback_matching_tree : t -> Stylable.t_style_selector -> Stylable.t_style_change_fn -> unit
  val add_style_rule : t -> Stylable.t_style_selector list -> (t_stylable_name * t_stylable_value) list -> unit
  ...
  let build_desc desc t =
    if (not (List.mem_assoc desc t.built_descs)) then
      (t.built_descs <- (desc,Stylable_desc_built.create desc t.ids)::t.built_descs);
  ...
  let add_style_defaults t nvis =
    let add_id_and_style n_v_i = 
      ...
      let stype = stype_of_svalue svalue in
  ...
  let apply_stylesheet t =
     ...
     *)
    List.iter (fun e -> Stylable.reset_next_values e) t.roots;

    (* Apply rules *)
    List.iter (fun r -> Style_rule.apply r t) t.rules;

    (* resolve next values to current values, determining which ones have changed,
    and callback if any have for each stylable
     *)
    List.iter (fun e -> Stylable.update_current_values_from_next e) t.roots;
    ()

  (*f element_callback_matching_tree *)
  let element_callback_matching_tree t selector callback =
    List.iter (fun e -> Stylable.element_callback_matching_tree selector e t callback) t.roots
